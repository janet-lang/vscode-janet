...
exports.createStringCursor = exports.LispTokenCursor = exports.TokenCursor = void 0;
...
class TokenCursor {
    ...
    atEnd() {
        return (this.line == this.doc.lines.length - 1 &&
 ...
 */
function _rangesForSexpsInList(cursor, useRowCol = false, openingBracket) {
...
class LispTokenCursor extends TokenCursor {
     ...
     */
    forwardSexp(skipComments = true, skipMetadata = false, skipIgnoredForms = false) {
            ...
            switch (token.type) {
                ...
                case 'close': {
                    ...
                    while ((open = stack.pop())) {
                        if ((0, janet_lexer_1.validPair)(open, close)) {
                            this.next();
                            break;
                        }
                    }
     ...
     * @returns true if the cursor was moved, false otherwise.
     */
    backwardSexp(skipComments = true, skipMetadata = false, skipIgnoredForms = false, skipReaders = true) {
        const stack = [];
     ...
     */
    forwardListOfType(closingBracket) {
        const cursor = this.clone();
     ...
     */
    backwardListOfType(openingBracket) {
        const cursor = this.clone();
     ...
     */
    rangeForList(depth, openingBracket) {
        const cursor = this.clone();
        let range = undefined;
     ...
     */
    rangeForCurrentForm(offset) {
        let afterCurrentFormOffset;
        ...
        if (afterCurrentFormOffset === undefined) {
            ...
            let isAdjacentBefore = tk.type === 'reader' ||
                this.tokenBeginsMetadata() ||
                pTk.type === 'reader' ||
    ...
    rangeForDefun(offset, commentCreatesTopLevel = true) {
        const cursor = this.doc.getTokenCursor(offset);
        let lastCandidateRange = cursor.rangeForCurrentForm(offset);
     ...
     * If you are particular about which list type that should be considered, supply an `openingBracket`.
     */
    rowColRangesForSexpsInList(openingBracket) {
        const cursor = this.clone();
        return _rangesForSexpsInList(cursor, true, openingBracket);
    }
    /**
     * Returns the rowCol ranges for all forms in the current list.
     * Returns undefined if the current cursor is not within a list.
     * If you are particular about which list type that should be considered, supply an `openingBracket`.
     */
    rangesForSexpsInList(openingBracket) {
        const cursor = this.clone();
        return _rangesForSexpsInList(cursor, false, openingBracket);
    }
    /**
...
function createStringCursor(s) {
    const model = new model_1.LineInputModel();
