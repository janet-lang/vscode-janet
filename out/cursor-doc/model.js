...
exports.StringDocument = exports.LineInputModel = exports.ModelEditSelection = exports.ModelEdit = exports.TextLine = exports.initScanner = void 0;
const janet_lexer_1 = require("./cdf-edits/janet-lexer");
const token_cursor_1 = require("./token-cursor");
...
function initScanner(maxLength) {
    scanner = new janet_lexer_1.Scanner(maxLength);
}
...
class LineInputModel {
    ...
    constructor(lineEndingLength = 1, document) {
    ...
    updateLines(start, deleted, inserted) {
        ...
        this.changedLines = new Set(Array.from(this.changedLines)
            .map((x) => {
            ...
            .filter((x) => x !== null));
        this.insertedLines = new Set(Array.from(this.insertedLines)
            .map((x) => {
            const [a, b] = x;
            ...
            .filter((x) => x !== null));
        this.deletedLines = new Set(Array.from(this.deletedLines)
            .map((x) => {
            const [a, b] = x;
            if (a > start && a < start + deleted) {
                return null;
     ...
     */
    getWordSelection(offset) {
        const stopChars = [' ', '"', ';', '.', '(', ')', '[', ']', '{', '}', '\t', '\n', '\r'], [row, column] = this.getRowCol(offset), text = this.lines[row].text;
            ...
            for (const edit of edits) {
                switch (edit.editFn) {
                    case 'insertString': {
                        ...
                        this.insertString(...edit.args.slice(0, 4));
                        break;
                    }
                    case 'changeRange': {
                        const fn = this.changeRange;
                        this.changeRange(...edit.args.slice(0, 5));
                        break;
                    }
                    case 'deleteRange': {
                        const fn = this.deleteRange;
                        this.deleteRange(...edit.args.slice(0, 5));
                        break;
                    }
                    default:
                        break;
    ...
    getTokenCursor(offset, previous = false) {
...
class StringDocument {
    constructor(contents) {
        this.model = new LineInputModel(1, this);
        this.selectionStack = [];
        if (contents) {
